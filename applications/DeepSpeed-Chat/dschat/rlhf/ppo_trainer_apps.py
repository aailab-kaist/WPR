...
from inference_apps import get_reward_score
...
class DeepSpeedPPOTrainer():
    ...
    def __init__(self, rlhf_engine, args):
        ...
        self.f_eval = self._get_f_divergence(self.args.f_div, self.args.f_div_alpha, self.args.f_div_low_var)

    def _get_f_divergence(self, f_divergence, alpha=0.5, low_var=False):
        if f_divergence == 'rkl':
            def f(logx):
                imp_f_val = logx    # 1/x * x logx
                if low_var:
                    x_inv = torch.exp(-logx)
                    return imp_f_val + x_inv - 1
                else:
                    return imp_f_val
            ...
            def f(logx):
                x = torch.exp(logx)
                x_inv = torch.exp(-logx)
                imp_f_val = logx - (1 + x_inv) * torch.log((x + 1) / 2)
                return imp_f_val
                # u = torch.exp(-logx)
                # f_val = u * (-logx) - (u+1) * torch.log((u+1) / 2)
                # return f_val
            ...
            def f(logx):
                x_inv = torch.exp(-logx)
                imp_f_val = (1.0 / (alpha * (alpha - 1))) * (torch.pow(x_inv, alpha) - alpha * x_inv - (1 - alpha))
                if low_var:
                    return imp_f_val
                else:
                    return imp_f_val
        elif f_divergence == 'tv':
            def f(logx):
                x = torch.exp(logx)
                x_inv = torch.exp(-logx)
                imp_f_val = 0.5 * x_inv * torch.abs(x - 1)
                if low_var:
                    return imp_f_val
                else:
                    return imp_f_val
        elif f_divergence == 'chi':
            def f(logx):
                x = torch.exp(logx)
                x_inv = torch.exp(-logx)
                imp_f_val = x_inv * (x - 1) ** 2
                if low_var:
                    return imp_f_val
                else:
                    return imp_f_val
    ...
    def _generate_sequence(self, prompts, mask, step):
        ...
        for i in range(batch_size):
            if valid_ans_len[
                ...
                out_prompt.append(prompts[i:i + 1])
        ...
        out_prompt = torch.cat(out_prompt, dim=0)  # concat output in the batch dim

        return out_seq, out_prompt

    def generate_experience(self, prompts, mask, step, meta=None):
        self.eval()
        generate_start = time.time()
        seq, prompts = self._generate_sequence(prompts, mask, step)
        ...
        with torch.no_grad():
            ...
            ans = seq[:, prompts.shape[-1]:]
            ans_token = self.tokenizer.batch_decode(ans, skip_special_tokens=True)
            prompts_token = self.tokenizer.batch_decode(prompts, skip_special_tokens=True)
            reward_score = get_reward_score(prompts_token, ans_token, meta)
            reward_score = torch.tensor(reward_score, dtype=values.dtype, device=values.device)
    ...
    def compute_rewards(self, prompts, log_probs, ref_log_probs, reward_score, action_mask, return_kl=False):
        ...
        log_probs_ratio = log_probs - ref_log_probs
        kl_divergence_estimate = -self.kl_ctl * self.f_eval(log_probs_ratio)
        rewards = torch.clone(kl_divergence_estimate)
        if self.args.f_div != ['rkl']:
            rewards = torch.clamp(rewards, -self.clip_reward_value, self.clip_reward_value)
        ...
        if return_kl:
            ...
            kl_divergence_estimate = kl_divergence_estimate.sum(dim=1) / (ends - start)
