...
class DeepSpeedRLHFWPOEngine():
    ...
    def _init_actor(self, actor_model_name_or_path):
        ...
        # Optimizer
        AdamOptimizer = DeepSpeedCPUAdam if ds_config["zero_optimization"]["offload_optimizer"]["device"] == "cpu" else FusedAdam
        ...
        if self.args.warmup_ratio is not None:
            num_warmup_steps = self.args.warmup_ratio * self.num_total_iters
        else:
            num_warmup_steps = self.args.num_warmup_steps
        print("Actor num_warmup_steps: ", num_warmup_steps, "num_total_iters: ", self.num_total_iters)
        # LR Scheduler
    ...
    def _init_critic(self, critic_model_name_or_path):
        ...
        # Model
        if 'qwen' in critic_model_name_or_path:
        ...
        if pad_id is not None:
            if getattr(critic_model, "config", None) is not None:
                print('='*30)
                print('critic_config')
                critic_model.config.pad_token_id = pad_id
            if hasattr(critic_model, "rwtransformer") and hasattr(critic_model.rwtransformer, "embed_tokens"):
                print('='*30)
                print('critic_pad')
                critic_model.rwtransformer.embed_tokens.padding_idx = pad_id
        ...
        if self.args.warmup_ratio is not None:
            ...
            num_warmup_steps = self.args.num_warmup_steps
        print("Critic num_warmup_steps: ", num_warmup_steps, "num_total_iters: ", self.num_total_iters)
        # LR Scheduler
        lr_scheduler = get_scheduler(
            name=self.args.lr_scheduler_type,
            optimizer=optim,
