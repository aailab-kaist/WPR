...
def get_raw_dataset(dataset_name, output_path, seed, local_rank, train_phase=-1):
    ...
    if "Dahoas/rm-static" in dataset_name:
        ...
        return raw_datasets.LmqgQagjaquadDataset(output_path, seed, local_rank,
                                                 dataset_name)
    elif "summarize_from_feedback" in dataset_name:
        return raw_datasets.OpenAISummarizeDataset(output_path, seed, local_rank,
                                              dataset_name)
    elif "codeparrot/apps" in dataset_name:
        return raw_datasets.CodeparrotAPPSDataset(output_path, seed, local_rank,
                                              dataset_name, train_phase)
...
class PromptDataset(Dataset):
    ...
    def __init__(self, prompt_dataset, chosen_dataset, reject_dataset,
                 pad_token_id, train_phase, apps=False) -> None:
        super().__init__()
        ...
        if self.train_phase == 1:
            ...
            if self.apps:
                return self.prompt_dataset[idx]["input_ids"],self.prompt_dataset[idx]["attention_mask"], \
                    self.prompt_dataset[idx]["meta"], self.pad_token_id
            else:
                return self.prompt_dataset[idx]["input_ids"],self.prompt_dataset[idx]["attention_mask"], \
                    self.pad_token_id
...
def create_dataset_split(current_dataset, raw_dataset, train_phase, tokenizer,
                         end_of_conversation_token, max_seq_len, apps=False):
    prompt_dataset = []
    chosen_dataset = []
    reject_dataset = []
    if train_phase == 1:
        ...
        for i, tmp_data in enumerate(current_dataset):
            ...
            if prompt is not None:
                ...
                if prompt_token["input_ids"].size()[-1] <= max_seq_len:
                    ...
                    if apps:
                        meta = raw_dataset.get_meta(tmp_data)
                        prompt_token['meta'] = meta
    ...
    return PromptDataset(prompt_dataset, chosen_dataset, reject_dataset,
                         tokenizer.pad_token_id, train_phase, apps=apps)


def create_dataset(local_rank, dataset_name, data_split, output_path,
                   train_phase, seed, tokenizer, end_of_conversation_token,
                   max_seq_len, rebuild):
    raw_dataset = get_raw_dataset(dataset_name, output_path, seed, local_rank, train_phase)
    ...
    train_dataset = create_dataset_split(train_dataset, raw_dataset,
                                         ...
                                         max_seq_len, apps='apps' in dataset_name)
...
class DataCollatorRLHF:
    ...
    def __init__(self, max_token_len, inference_tp_size, apps=False):
