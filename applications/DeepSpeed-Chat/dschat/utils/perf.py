...
# Enhanced version of the function above that provides calculations and printing for Step 3
def print_throughput_step3_w(actor_model,
                           critic_model,
                           critic_w_model,
                           args,
                           e2e_time,
    ...
    if rank <= 0:
        ...
        # Critic model passed here is  a DeepSpeed Engine. The module inside is the Reward model (that wraps a HF model).
        critic_hf_config = critic_model.module.config
        critic_w_hf_config = critic_w_model.module.config

        actor_num_layers, actor_hidden_size, actor_vocab_size = get_hf_configs(
            actor_hf_config)
        critic_num_layers, critic_hidden_size, critic_vocab_size = get_hf_configs(
            critic_hf_config)
        critic_w_num_layers, critic_w_hidden_size, critic_w_vocab_size = get_hf_configs(
            critic_w_hf_config)
        ...
        actor_checkpoint_activations_factor = 4 if args.actor_gradient_checkpointing else 3
        critic_checkpoint_activations_factor = 4 if args.critic_gradient_checkpointing else 3
        critic_w_checkpoint_activations_factor = 4 if args.critic_gradient_checkpointing else 3
        # critic_w_checkpoint_activations_factor = 4 if args.critic_w_gradient_checkpointing else 3
        if args.actor_lora_dim > 0:
            k = args.actor_lora_dim * 2 / actor_hidden_size
            actor_checkpoint_activations_factor -= (1 - k)
        if args.critic_lora_dim > 0:
            k = args.critic_lora_dim * 2 / critic_hidden_size
            critic_checkpoint_activations_factor -= (1 - k)
        # if args.critic_w_lora_dim > 0:
            k = args.critic_lora_dim * 2 / critic_w_hidden_size
            critic_w_checkpoint_activations_factor -= (1 - k)
        ...
        critic_w_model._num_params = sum([
            p.ds_numel if hasattr(p, "ds_tensor") else p.numel()
            for p in critic_w_model.parameters()
        ])
        critic_w_params_in_billions = critic_w_model._num_params / (1e9)
        ...
        critic_train_flops_per_iteration = calculate_flops(
            critic_checkpoint_activations_factor, batch_size, seq_length,
            critic_hf_config)
        critic_w_train_flops_per_iteration = calculate_flops(
            critic_w_checkpoint_activations_factor, batch_size, seq_length,
            critic_w_hf_config)

        total_train_flops = actor_train_flops_per_iteration + critic_train_flops_per_iteration + critic_w_train_flops_per_iteration
        train_tflops = total_train_flops / (train_time * gpus_per_model *
                                            (10**12))
        ...
        actor_param_string = f"{actor_params_in_billions:.3f} B" if actor_params_in_billions != 0 else "NA"
        critic_param_string = f"{critic_params_in_billions:.3f} B" if critic_params_in_billions != 0 else "NA"
        critic_w_param_string = f"{critic_w_params_in_billions:.3f} B" if critic_w_params_in_billions != 0 else "NA"
        print(
            f"Actor Model Parameters => {actor_param_string}, Critic Model Parameters => {critic_param_string}, Critic W Model Parameters => {critic_w_param_string}"
        )
