...
def save_model(step, args, tokenizer, rm_model):
    print_rank_0(f'saving model for step {step}...', args.global_rank)
    rm_model = convert_lora_to_linear_layer(rm_model)

    if args.global_rank == 0:
        save_hf_format(rm_model, tokenizer, args, sub_folder=f"step_{step}")
...
def main():
    ...
    if args.warmup_ratio is not None:
        args.num_warmup_steps = args.warmup_ratio * (args.num_train_epochs * num_update_steps_per_epoch)
