...
from dschat.rlhf.ppo_wpr_trainer import DeepSpeedWPOTrainer
from dschat.rlhf.rlhf_wpr_engine import DeepSpeedRLHFWPOEngine
...
def parse_args():
    ...
    parser.add_argument(
        "--normalize",
        action='store_true',
        help='Sinkhorn dual variable normalize')
    parser.add_argument(
        "--gaussian_perturb",
        type=float,
        default=0.)
...
def main():
    ...
    if args.end_of_conversation_token != "None":
    ...
    rlhf_engine = DeepSpeedRLHFWPOEngine(
    ...
    ppo_trainer = DeepSpeedWPOTrainer
    ...
    # Sparse cost matrix
    if args.cost_type == 'L2':
        cost_type = ''
    else:
        cost_type = f"{args.cost_type}_"
    if args.cost_emb == 'summarize':
        cost_emb = ''
    else:
        cost_emb = f"{args.cost_emb}_"
    cost_save_path = f'cost_{cost_emb}{cost_type}{args.cost_dim}.pt'
    indices_save_path = f'indices_{cost_emb}{cost_type}{args.cost_dim}.pt'
    if not os.path.exists(cost_save_path):
        embeddings = trainer.ref_model.model.embed_tokens.weight
        embeddings = embeddings.data.detach()

        if args.gaussian_perturb != 0:
            embeddings = (1 - args.gaussian_perturb) * embeddings + torch.randn_like(embeddings) * args.gaussian_perturb
        ...
        for start in range(0, N, block_size):
            ...
            if args.cost_type == 'L2':
                dist = torch.cdist(block, embeddings, p=2)
            elif args.cost_type == 'cos':
                eps = 1e-8
                block_n = torch.nn.functional.normalize(block, p=2, dim=1, eps=eps)
                emb_n = torch.nn.functional.normalize(embeddings, p=2, dim=1, eps=eps)
                dist = 1 - block_n @ emb_n.T
            else:
                raise NotImplementedError(f"Unknown cost type: {args.cost_type}")
            distances, indices = torch.topk(dist, k + 1, largest=False)
            ...
            for i in range(end - start):
                ...
                for j in range(k):
                    rows.append(src)
                    cols.append(indices[i, j].item())
                    vals.append(distances[i, j].item())
            print_rank_0(f"Cost matrix construction progress: {start / N * 100} %...")

        C = sparse.csr_matrix((vals, (rows, cols)), shape=(N, N))
        coo = C.tocoo()

        rows_np = coo.row.astype(np.int64)
        cols_np = coo.col.astype(np.int64)

        indices_np = np.vstack([rows_np, cols_np])
        indices = torch.from_numpy(indices_np).to(device)
        costs = torch.from_numpy(coo.data.astype(np.float32)).to(device)
        torch.save(costs, cost_save_path)
        torch.save(indices, indices_save_path)
    print_rank_0(f"cost save path: {cost_save_path}")
    epsilon = args.epsilon
    costs = torch.load(cost_save_path, map_location=device)
    indices = torch.load(indices_save_path, map_location=device)
    N = trainer.ref_model.model.embed_tokens.weight.shape[0]
    K_vals = torch.exp(-costs / epsilon)
    K = torch.sparse_coo_tensor(indices, K_vals, size=(N, N))

    def symmetrize_and_selfloop(coo: torch.sparse_coo_tensor, self_val: float = 1.0) -> torch.sparse_coo_tensor:
        """
        Given a sparse COO tensor of shape (L, L), add symmetric entries and self-loops.
        - For each (i,j,val), ensure (j,i,val) exists.
        - For each i, set (i,i) = self_val (override any existing).
        If duplicates occur, take the maximum value.
        """
        coo = coo.coalesce()
        idx = coo.indices()  # 2 x nnz
        vals = coo.values()  # nnz
        L = coo.size(0)
        ...
        # symmetric entries
        sym_i = torch.cat([i_idx, j_idx], dim=0)
        sym_j = torch.cat([j_idx, i_idx], dim=0)
        sym_v = torch.cat([v, v], dim=0)

        # self-loop entries
        self_idx = torch.arange(L, device=v.device)
        self_i = torch.cat([self_idx, ], dim=0)
        self_j = torch.cat([self_idx, ], dim=0)
        self_v = torch.full((L,), self_val, device=v.device)

        # combine all
        all_i = torch.cat([sym_i, self_i], dim=0)
        all_j = torch.cat([sym_j, self_j], dim=0)
        all_v = torch.cat([sym_v, self_v], dim=0)

        # unique (i,j) with max value
        coords = torch.stack([all_i, all_j], dim=1)  # (total, 2)
        unique_coords, inv = torch.unique(coords, dim=0, return_inverse=True)
        # reduce to max value per unique
        max_v = torch.zeros(unique_coords.size(0), device=v.device)
        max_v = max_v.scatter_reduce(0, inv, all_v, reduce='amax', include_self=True)

        new_idx = unique_coords.t()  # 2 x nnz_new
        return torch.sparse_coo_tensor(new_idx, max_v, (L, L), device=v.device)

    K = symmetrize_and_selfloop(K).coalesce()
    Kt = K.transpose(0, 1)
            ...
            if exp_dataset is not None:
                ...
                for ppo_ep in range(args.ppo_epochs):
                    for i, exp_data in enumerate(exp_dataset):
                        actor_loss, critic_loss, kl_divergence = trainer.train_rlhf(exp_data, constituent_tree, K, Kt, epsilon=args.epsilon, max_iter=args.max_iter, tol=args.tol, top_k=args.top_k, normalize=args.normalize)
                ...
                print_rank_0(
                    f"Average reward score: {average_reward/inner_iter} | EMA reward score: {ema_reward_score.get()} | Average Wass. {average_kl} | Average Response Length: {resp_length}",
                    args.global_rank)
                print_rank_0(
                    "-------------------------------------------------------------------------------------",
                    args.global_rank)
