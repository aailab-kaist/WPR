...
def get_gpt_response(query, sk, max_tokens=5):
...
def main(args):
    ...
    start_offset = 0
    tbar = tqdm(range(len(model_a_resp[start_offset: args.num_sample])))
    ...
    for idx in tbar:
        ...
        if reverse_flag:
            if args.dataset == 'summarize':
                ...
                max_tokens = 100
            resp = get_gpt_response(inst, args.sk, max_tokens=max_tokens)
