...
def get_reward_score(reward_model, tokenizer, prompts, responses, prompt_length, model='gemma'):
    # if 'qwen' in model:
...
def main(args):
    ...
    with torch.no_grad():
        if args.no_actor_folder:
            tokenizer = AutoTokenizer.from_pretrained(f"{args.model}", device_map=f'cuda:{args.gpu + 1}')
            model = AutoModelForCausalLM.from_pretrained(f"{args.model}", device_map=f"cuda:{args.gpu + 1}", torch_dtype=torch.float16)
        else:
        ...
        for i in tqdm(range(0, len(prompts), batch_size)):
            ...
            if args.ref_scores:
                chosen_reward_score = get_reward_score(reward_model, tokenizer, prompts[i: i + batch_size], chosens[i: i + batch_size], 2, args.model)
                rejected_reward_score = get_reward_score(reward_model, tokenizer, prompts[i: i + batch_size], rejecteds[i: i + batch_size], 2, args.model)
                ...
                reward_score = get_reward_score(reward_model, tokenizer, prompts[i: i + batch_size], response, 2, args.model)
    ...
    if args.ref_scores:
        with open(f'{PROJ_PATH}/results/{args.dataset}/temperature={args.temperature}/dataset.jsonl', 'w') as f:
            for inference in inferences:
                f.write(json.dumps(inference) + '\n')
    else:
        with open(f'{PROJ_PATH}/results/{args.dataset}/temperature={args.temperature}/{model_name}_{step}.jsonl', 'w') as f:
            for inference in inferences:
                f.write(json.dumps(inference) + '\n')
